document.addEventListener("DOMContentLoaded", function () {
  ...
  if (typeof auth === "undefined" || typeof db === "undefined") {
    console.error(
      "Firebase 'auth' or 'db' object is not defined. Check your script loading order and Firebase config in HTML.",
    );
    alert(
      "Sign Up Service Error: Firebase services are not initialized. Please check console for details.",
    );
    return;
  }
  ...
  async function handleTeacherSignup(event) {
    ...
    try {
      ...
      console.log(
        "User data saved to Firestore successfully. Redirecting to teacher homepage...",
      );
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        message =
          "Sign up succeeded, but saving profile data failed (Permission Denied). Check Firestore Security Rules.";
      }

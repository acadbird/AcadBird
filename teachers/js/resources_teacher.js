  ...
  function initEventListeners() {
    ...
    document.querySelectorAll(".week-header").forEach((header) => {
      header.onclick = (event) => {
        ...
        if (
          ...
          !event.target.closest(".delete-week-icon") &&
          !event.target.closest(".lock-status")
        ) {
    ...
    document.querySelectorAll(".topic-header").forEach((header) => {
      header.onclick = (event) => {
        ...
        if (
          !event.target.closest(".edit-icon") &&
          !event.target.closest(".delete-topic-icon") &&
          !event.target.closest(".lock-status")
        ) {
    ...
    document
      .querySelectorAll(".week-card .lock-status, .topic-card .lock-status")
      .forEach((lockStatus) => {
        lockStatus.onclick = (event) => {
          event.stopPropagation();
          const card =
            lockStatus.closest(".week-card") ||
            lockStatus.closest(".topic-card");
    ...
    document
      .querySelectorAll(".week-edit-icon, .topic-edit-icon")
      .forEach((icon) => {
        icon.onclick = (event) => {
          const parent =
            event.target.closest(".week-header") ||
            event.target.closest(".topic-header");
          let textSpan;
          ...
          if (textSpan) {
            ...
            const saveEdit = () => {
              const newText =
                inputField.value || (isWeekSubtitle ? "New Week" : "New Topic");
              const newSpan = document.createElement("span");
              newSpan.textContent = newText;
              newSpan.classList.add(
                isWeekSubtitle ? "week-subtitle" : "topic-title",
              );
    ...
    document
      .querySelectorAll(".topic-item.content-item-editable")
      .forEach((item) => {
        item.onclick = (event) => {
          if (
            !event.target.closest(".edit-item-icon") &&
            !event.target.closest(".delete-item-icon")
          ) {
    ...
    document.querySelectorAll(".delete-week-icon").forEach((icon) => {
      icon.onclick = (event) => {
        ...
        if (
          confirm(
            "Are you sure you want to delete this entire week? This action cannot be undone.",
          )
        ) {
          weekCard.remove();
        }
